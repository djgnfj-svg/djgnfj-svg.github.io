...
진입장벽이 낮은 영역에 AI를 붙여 빠르게 수익을 만들고 싶었다. 그 첫 번째 대상으로 한국 주식을 선택했다. 60건 이상의 크롤링 외주를 하면서 데이터를 수집하고 가공하는 일에는 이미 익숙했고, 주식 데이터는 정형화되어 있어 그 경험을 바로 적용할 수 있는 도메인이었기 때문이다.

처음에는 단순하게 생각했다. 프로젝트를 열고 Claude에게 시키면 되니까. 수집 자체는 어렵지 않았다. 문제는 그 이후였다. 야근을 하거나 잠시 놓친 날에 데이터 공백이 생겼고, 잠들기 전에 컴퓨터를 켜두자니 소음이 거슬렸다. 클라우드에 올리면 해결되지만, 아직 수익으로 성적이 증명되지 않은 단계에서 인프라 비용을 태우고 싶지 않았다. 결국 로컬에서 먼저 파이프라인을 구축하고 검증한 뒤, 성과가 확인되면 클라우드로 올리기로 했다. 현재는 AWS로 이전을 준비 중이다. 수집부터 AI 판단까지 전체 흐름을 파이프라인으로 묶는 것이 첫 번째 과제였다.
...
이 구조로 설계한 핵심 이유는 **각 단계의 실패를 독립적으로 추적하고 복구할 수 있어야** 했기 때문이다. 수집이 실패해도 어제 적재된 데이터로 AI 판단은 돌릴 수 있고, AI 판단이 실패해도 수집·적재는 정상 완료로 기록된다. DAG를 4개로 분리한 이유도 같다. 수집과 예측의 실행 시점과 실패 특성이 다르기 때문에 하나의 DAG에 몰아넣으면 운영이 복잡해진다.
...
소스를 6개로 분산한 이유는 단일 소스 의존 리스크를 줄이기 위해서다. pykrx가 장애를 일으켜도 yfinance로 기본 시세는 확보할 수 있고, Naver API가 막혀도 Tavily로 뉴스를 가져올 수 있다.
...
`ThreadPoolExecutor`를 쓴 이유는 pykrx/yfinance 내부가 동기 HTTP 호출이라 `asyncio`의 이점이 제한적이기 때문이다. I/O 대기 시간을 스레드로 겹치는 것만으로 충분했다. 배치 간 딜레이는 API rate limit 대응이다.
...
처음에는 Cron + 쉘 스크립트로 시작했다. 수집 → 검증 → 적재가 순차적으로 실행되어야 하는데, Cron에서는 "앞 단계가 성공했을 때만 다음 단계 실행"을 구현하려면 lock 파일이나 상태 파일을 직접 관리해야 했다. 실패 시 재시도도 직접 구현해야 했고, 어떤 Task가 언제 실패했는지 추적하기 어려웠다.

Airflow를 선택한 결정적 이유는 세 가지다:

1. **Task 간 의존성을 코드로 선언**할 수 있다. `>>` 연산자 한 줄로 "수집이 끝나야 검증이 시작된다"를 표현한다.
2. **실패 재시도와 알림이 내장**되어 있다. `retries`, `retry_delay`, `on_failure_callback`을 설정만 하면 된다.
3. **실행 이력과 로그를 웹 UI에서 확인**할 수 있다. 새벽에 실행된 파이프라인이 실패했을 때, 아침에 UI에서 어떤 Task가 왜 실패했는지 바로 확인할 수 있다.

Prefect나 Dagster도 검토했다. Prefect는 러닝 커브가 낮지만 Prefect Cloud 의존도가 높았고, 로컬/온프레미스 운영에서 레퍼런스가 부족했다. Dagster는 Asset 기반 패러다임이 흥미로웠으나, 이 프로젝트의 Task 기반 흐름에는 Airflow의 모델이 더 직관적이었다.
...
**왜 하나의 DAG에 몰아넣지 않았는가**: 수집(daily_collect)과 예측(daily_predict)의 실행 시점이 다르다. 수집은 장 마감 직후(19:30)에, 예측은 다음 날 새벽(05:00)에 실행된다. 하나의 DAG로 묶으면 수집 후 예측까지 9시간을 기다리는 비효율적인 구조가 된다. 또한 LLM API 장애로 예측이 실패했을 때, 수집까지 재실행되는 것을 방지할 수 있다.

**daily_verify가 존재하는 이유**: AI 판단의 정확도를 추적하지 않으면, 모델이 잘하고 있는지 나빠지고 있는지 알 수 없다. 매일 전일 예측과 실제 결과를 비교하여 정확도를 기록한다. 이 데이터가 쌓이면 weekly_backtest에서 전략 개선의 근거가 된다.
...
재시도를 2회로 설정한 이유: API 일시 장애는 대부분 10분 내에 복구된다. 3회 이상이면 근본적인 문제(API 변경, 인증 만료)일 가능성이 높아 알림을 보내는 게 낫다.

**`trigger_rule` 활용**: 알림 Task의 `trigger_rule=TriggerRule.ALL_DONE`이 핵심이다. 기본값 `ALL_SUCCESS`는 앞 Task가 모두 성공해야 실행되지만, `ALL_DONE`은 성공이든 실패든 앞 Task가 완료되면 실행된다. 파이프라인이 실패했을 때 알림이 안 가면 의미가 없기 때문이다.
...
**Connection & Variable 관리**: DART API 키, ECOS API 키, Naver CLIENT_ID/SECRET, Tavily API 키 등 민감 정보가 많다. 코드에 하드코딩하지 않고 Airflow의 Connection과 Variable로 관리한다.

```python
from airflow.hooks.base import BaseHook

conn = BaseHook.get_connection('postgres_stock')
...
로컬 환경이라 리소스가 제한적이다. 기본값 그대로 두면 수집 Task 5개가 동시에 각각 ThreadPoolExecutor를 돌리면서 메모리가 부족해지는 문제가 있었다. `parallelism`과 `dag_concurrency`를 낮춰서 해결했다.

**실제 겪은 운영 문제들**:

1. **Scheduler 메모리 누수**: Airflow 2.x에서 Scheduler가 DAG 파싱을 반복하며 메모리가 점진적으로 증가했다. `AIRFLOW__SCHEDULER__NUM_RUNS`를 `1000`으로 설정해서 주기적으로 Scheduler를 재시작하도록 했다.

2. **Task가 stuck 상태로 멈춤**: 네트워크 장애로 Task가 응답 없이 멈추는 경우가 있었다. `execution_timeout`을 설정하지 않으면 영원히 running 상태로 남는다. 모든 Task에 적절한 타임아웃을 설정하는 것이 필수다.

3. **DB 백엔드 교체**: 처음에 Airflow 메타데이터 DB로 SQLite를 사용했는데, `SequentialExecutor`만 사용 가능해서 Task 병렬 실행이 불가능했다. PostgreSQL로 교체하고 `LocalExecutor`를 사용해 해결했다. 수집 Task 5개를 병렬로 돌려야 하는 이 프로젝트에서 `SequentialExecutor`는 선택지가 아니었다.
...
**왜 3계층인가**: raw를 보존하는 이유는 60건의 외주에서 뼈저리게 배운 교훈이다. 파서를 수정한 뒤 과거 데이터를 재처리할 수 없어서 데이터 공백이 생긴 적이 있다. raw에 원본을 JSONB로 통째 저장해두면 staging의 변환 로직을 바꿔도 언제든 재처리할 수 있다.

staging과 mart를 분리한 이유는 관심사의 분리다. staging은 "정확하고 깨끗한 데이터", mart는 "AI 에이전트가 바로 소비할 수 있는 형태"다. staging의 뉴스 감성 점수를 mart에서 종목별 일별 평균으로 집계하는 식이다.
...
raw에 JSONB로 저장하는 이유: Naver API와 Tavily API의 응답 구조가 다르다. 공통 스키마로 강제하면 각 API의 고유 필드를 잃는다. JSONB로 통째 저장하고, staging 변환 단계에서 각 소스별 파서가 필요한 필드만 추출한다.
...
**인덱싱 전략**: `(symbol, date)` 복합 PK 외에 `date`와 `symbol` 단독 인덱스를 추가했다. "특정 날짜의 전 종목 데이터"와 "특정 종목의 히스토리" 조회가 모두 빈번하기 때문이다. 파티셔닝은 현재 규모(일일 수천 건, 누적 수십만 건)에서는 불필요하다고 판단했다. 파티셔닝의 관리 복잡도 대비 성능 이점이 크지 않은 규모다. 누적 데이터가 수백만 건을 넘기면 date 기준 레인지 파티셔닝을 검토할 계획이다.
...
처음에는 규칙 기반으로 시작했다. "RSI 30 이하면 매수 신호", "PER 업종 평균 이하면 저평가" 같은 규칙이다. 문제는 지표들이 서로 충돌할 때다. RSI는 매수 신호인데 거시 경제 지표는 침체를 가리키는 상황에서, 규칙 기반은 if-else가 폭발적으로 늘어난다. 결국 **다양한 데이터를 종합적으로 해석하는 능력**이 필요했고, 이건 LLM이 잘하는 영역이다.

단순 LLM API 호출이 아닌 LangGraph를 선택한 이유는, 분석이 단일 질문-답변이 아니라 **멀티스텝 토론**이어야 했기 때문이다. 한 명의 분석가에게 모든 걸 맡기면 확증 편향에 빠질 수 있다. 서로 다른 관점의 에이전트가 순차적으로 분석하고, 마지막에 종합 판단을 내리는 구조가 더 견고하다.
...
**왜 4개 에이전트인가**: 핵심은 **Red Team**의 존재다. Quant와 Qual이 같은 방향의 결론을 내면 확증 편향이 생긴다. Red Team이 의도적으로 반론을 제기함으로써 Judge가 양쪽 논거를 모두 고려한 판단을 내릴 수 있다. 이는 실제 투자 기관에서 사용하는 "Devil's Advocate" 프로세스를 에이전트 그래프로 구현한 것이다.

각 노드를 분리한 또 다른 이유는 **프롬프트 전문화**다. 하나의 프롬프트에 모든 역할을 넣으면 분석 깊이가 얕아진다. 역할을 분리하면 각 노드의 프롬프트를 독립적으로 개선할 수 있고, 어느 단계에서 분석이 부정확한지 디버깅하기도 쉽다.
...
def run_langgraph_agent(**context):
    ...
    try:
        ...
        # fallback: 전일 판단 유지 + 알림
        fallback_to_previous_judgment(context['execution_date'])
        send_slack_alert(f"AI 판단 실패 — 전일 판단 유지: {e}")
        raise
```
...
**Fallback 처리**: AI 판단 실패 시 전일 판단을 유지한다. "판단 없음"보다 "어제와 동일"이 운영상 더 안전하다. 동시에 Slack 알림을 보내 수동 확인을 유도한다.
...
**사례 1: pykrx 데이터 지연**: 장 마감 직후 pykrx 데이터가 아직 반영되지 않아 빈 데이터가 수집된 적이 있다. 건수 체크 검증에서 감지되었고, 수집 스케줄을 장 마감 4시간 후(19:30)로 조정하여 해결했다.

**사례 2: LLM API 응답 지연**: GPT-4 API 서버 과부하로 AI 판단 Task가 30분 타임아웃에 걸렸다. fallback 로직이 전일 판단을 유지했고, Slack 알림으로 즉시 인지할 수 있었다.

**사례 3: PostgreSQL 디스크 풀**: 데이터가 쌓이면서 디스크가 가득 찼다. "could not extend file" 에러가 발생했다. 30일 이상 지난 raw 데이터를 아카이빙하는 배치를 추가하고, Prometheus에서 디스크 사용률 80% 이상 시 사전 알림을 보내도록 설정했다.
...
Airflow 자체의 학습보다 **Airflow를 안정적으로 운영하는 것**이 어려웠다. DAG 작성은 문서만 보면 되지만, Scheduler 메모리 누수, Task stuck, 메타데이터 DB 성능 저하 같은 문제는 운영해봐야 겪는다. "Airflow를 쓸 줄 안다"와 "Airflow를 운영할 줄 안다"는 다른 차원의 역량이라고 느꼈다.

### Airflow에 대한 솔직한 평가

**좋았던 점**: Task 의존성 관리, 실패 재시도, 웹 UI를 통한 모니터링은 기대 이상이었다. 특히 새벽에 실행된 파이프라인의 실패 원인을 아침에 UI에서 바로 확인할 수 있는 것이 운영 효율에 크게 기여했다.

**아쉬웠던 점**: 로컬 환경에서의 리소스 사용량이 생각보다 크다. Scheduler, Webserver, PostgreSQL, Prometheus, Grafana까지 6개 컨테이너를 띄우면 유휴 상태에서도 메모리를 2~3GB 사용한다. 개인 프로젝트 규모에서는 다소 무거운 것이 사실이다.

### LangGraph 4-에이전트 토론의 성과와 한계

**성과**: Red Team 에이전트의 도입이 가장 효과적이었다. Quant와 Qual이 모두 "매수"를 외칠 때, Red Team이 "외국인 순매도 추세가 3일째 이어지고 있다"며 반론을 제기하는 식이다. Judge가 이를 반영해 "매수"에서 "중립"으로 보수적으로 조정한 경우가 실제로 있었고, 사후 검증에서 그 판단이 더 정확했다.

**한계**: LLM의 판단은 결정론적(deterministic)이지 않다. 같은 데이터를 넣어도 다른 판단이 나올 수 있다. `temperature=0`으로 설정해도 완전히 동일하지는 않다. 또한 LLM이 사후적 합리화(post-hoc rationalization)를 하는지, 실제 데이터 기반 추론인지 구분하기 어렵다. `daily_verify` DAG로 사후 검증을 하는 것이 현재로서는 최선의 보완책이다.
...
60건의 크롤링 외주는 "데이터를 가져오는 기술"이었다. 이 프로젝트는 "가져온 데이터를 시스템으로 만드는 기술"이었다. 크롤링 스크립트를 짜는 것과 파이프라인을 설계·운영하는 것은 근본적으로 다른 역량이다. 전자가 "낚시하는 법"이라면, 후자는 "어장을 관리하는 법"이다. 이 프로젝트를 통해 수집 이후의 세계 — 정제, 적재, 자동화, 모니터링, 장애 대응 — 에 눈을 뜨게 되었다.
