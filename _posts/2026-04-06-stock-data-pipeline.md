...
주식은 첫 번째 도메인일 뿐이다. 이후 다른 영역으로 확장할 때도 수집 → 정제 → 적재 → 판단의 파이프라인 구조는 동일하다. 도메인이 바뀌어도 오케스트레이션 레이어를 재활용할 수 있어야 했고, 범용적인 워크플로우 도구가 필요했다.
...
```python
with DAG('daily_collect', schedule='30 10 * * 1-5',  # KST 19:30
         catchup=False, max_active_runs=1) as dag:

    [collect_prices, collect_fundamentals, collect_economic,
     collect_news, collect_global] >> transform >> load >> notify
```

수집 5개 Task를 병렬로 실행한 뒤, 정제 → 적재 → 알림 순으로 이어진다. 알림 Task는 `trigger_rule=TriggerRule.ALL_DONE`으로 성공이든 실패든 항상 실행된다.

**Task 분리 기준**: 각 Task가 독립적으로 실패하고 재시도될 수 있어야 한다. DART API만 장애일 때 전체를 재실행할 필요가 없다.

**`max_active_runs=1`**: 이전 실행이 끝나기 전에 다음이 시작되면 데이터가 꼬인다.

**`catchup=False`**: 밀린 스케줄을 한꺼번에 실행하지 않는다.

### 실행 전략: 재시도와 실패 처리

모든 Task에 `retries=2`, `retry_exponential_backoff=True`(5분 → 10분 → 20분)를 설정했다. API 일시 장애는 대부분 10분 내에 복구된다. 3회 이상이면 근본적인 문제일 가능성이 높아 알림을 보내는 게 낫다.
...
| 검증 | 조건 | 알림 |
|------|------|------|
| 건수 체크 | 예상 건수의 90% 미만 | 적재 건수 부족 |
| null 비율 | close 필드 null > 5% | 필수 필드 이상 |
| 전일 대비 급변 | 30% 이상 변동 종목 > 10건 | 파싱 오류 가능성 |

한국 주식은 가격제한폭이 ±30%이므로, 30% 초과 변동은 파싱 오류일 가능성이 높다.
...
`Quantitative(시세+기술지표)` → `Qualitative(뉴스+거시+재무)` → `Red Team(반론)` → `Judge(최종 판단)`
...
Airflow Task 안에서 PostgreSQL의 최신 데이터를 조회하여 LangGraph에 입력으로 전달한다. `execution_timeout=30분`으로 상한을 두었고, 4개 노드 순차 실행 시 전체 10~15분이 정상 범위다.

**Fallback 처리**: 이 시스템은 아침에 매수하고 저녁에 매도하는 데이트레이딩 전략이다. AI 판단이 실패하면 **오늘 투자 자체를 스킵**한다. 불확실한 판단으로 매수하는 것보다 하루를 쉬는 것이 리스크 관리상 더 안전하다. Slack으로 알림을 보내고, 수집·적재 데이터는 정상 보존되어 다음 날 판단에 활용된다.
...
AI 판단 결과는 `mart.ai_judgments` 테이블에 저장한다. 신호(매수/매도/중립), 확신도, 4개 에이전트 각각의 분석 텍스트, 사용 모델명, 토큰 사용량을 기록한다. `daily_verify` DAG가 이 데이터와 실제 주가 변동을 비교하여 정확도를 추적한다.
